...
class ImageProcessing extends PApplet {
  ...
  boolean kalmanFilter = false;
  List<KalmanFilter2D> KF2D = new ArrayList<KalmanFilter2D>();
  ...
  void setup() {
    ...
    twoD = new TwoDThreeD(img.width, img.height, imgType == 0 ? 0 : frameRate);
  ...
  void draw() {
    ...
    if (imgType == 1) img = readCamera();
    else if (imgType == 2) img = readVideo();
    ...
    if(!kalmanFilter) displayLines(lines, img);
    ...
    if (!quad.isEmpty()) {
      ...
      PVector rot = twoD.get3DRotations(quad);
      rotations = rotRange(rot);
    }
  }
  ...
  void kalman() {
    if (frameCount%kalmanRate == 0 || forcedCorrect) {
      ...
      if (quad.size() == 4) {
        for (int i = 0; i < 4; i++) {
          KF2D.get(i).predict_and_correct(quad.get(i));
        }
        forcedCorrect = false;
      } else {
    ...
    List<PVector> pred = new ArrayList<PVector>();
    for (int i = 3; i >= 0; i--) {
      pred.add(KF2D.get(i).dhat());
    }
    quad = pred;
  }
  ...
  PVector rotRange(PVector rot) {
    PVector newRot = rot.copy();
    if (rot.x > PI/2) newRot.x -= PI;
    else if (rot.x < -PI/2) newRot.x += PI;
    if (rot.y > PI/2) newRot.y -= PI;
    else if (rot.y < -PI/2) newRot.y += PI;
    if (rot.z > PI/2) newRot.z -= PI;
    else if (rot.z < -PI/2) newRot.z += PI;
    return newRot;
  }
